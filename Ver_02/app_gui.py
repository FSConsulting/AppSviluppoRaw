...
class AppSviluppoRaw:
    ...
    def gestisci_ridimensionamento_finestra(self, event):
        ...
        self._resize_debounce_id = self.root.after(200, self.aggiorna_anteprima)
    ...
    def _renderizza_anteprima_gui(self, img_ant, w_v, h_v):
        ...
        self.canvas_foto.config(scrollregion=(0, 0, w_v, h_v))
        self.tk_foto = ImageTk.PhotoImage(img_ant)
        
        # ANCORAGGIO NORD-OVEST RIGIDO (0,0): Impedisce scivolamenti e sfasamenti
        self.canvas_foto.create_image(0, 0, image=self.tk_foto, anchor=tk.NW)
        
        self.disegna_indicatori_macchie()
        self.canvas_foto.bind("<Configure>", self.gestisci_ridimensionamento_finestra)
        if self.in_zoom: self.root.after(10, self._esegui_centratura_post_render)
        else: self.set_status("Anteprima aggiornata")
    ...
    def apri_lightbox(self):
        ...
        """
        if not self.img_anteprima_base or self.tk_foto is None:
        ...
        self.lightbox_window.bind("<Button-1>", lambda event: self.lightbox_window.destroy())
        self.lightbox_window.transient(self.root)
        self.lightbox_window.grab_set()

        lbl_lightbox = tk.Label(self.lightbox_window, image=self.tk_foto, bg="black")
        lbl_lightbox.image = self.tk_foto
        lbl_lightbox.pack(expand=True)
        self.set_status("Lightbox aperto. Premi Esc o clicca per chiudere.")
